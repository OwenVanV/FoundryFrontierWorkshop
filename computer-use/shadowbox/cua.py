...
class Agent:
    ...
    def __init__(self, client, model, computer, run_logger=None):
        self.client = client
        self.model = model
        self.computer = computer
        self.state = None
        self.run_logger = run_logger
        self.last_screenshot = None  # base64 PNG of the most recent screenshot
    ...
    def continue_task(self, user_message="", instructions=None):
        ...
        if self.state.next_action == "computer_call_output":
            ...
            if self.run_logger:
                self.run_logger.log_actions(self.state.computer_actions)
            ...
            if self.run_logger and screenshot:
                self.run_logger.log_screenshot(screenshot, label=f"step_{self.run_logger.step_counter}")
        if self.state.next_action == "computer_call_output":
