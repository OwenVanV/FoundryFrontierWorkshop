...
class DockerComputer:
    ...
    def __init__(
        self,
        container_name="shadowboxer-vnc",
    ...
    def scroll(self, x: int, y: int, scroll_x: int, scroll_y: int) -> None:
        """
        Approximate pixel-based scroll_y into mouse wheel clicks using xdotool.
        """
        self._exec(f"DISPLAY={self.display} xdotool mousemove {x} {y}")
        
        # 1 scroll "click" ~= 70 pixels (adjust as needed)
        PIXELS_PER_CLICK = 100
        max_clicks = 5  # Prevent runaway scrolling

        clicks = min(max_clicks, max(1, abs(scroll_y) // PIXELS_PER_CLICK))
    ...
    def drag(self, path: list[dict[str, int]]) -> None:
        if not path:
            return
        # Support both object-attribute (.x) and dict-key (['x']) access
        def _get_xy(point):
            if isinstance(point, dict):
                return point["x"], point["y"]
            return point.x, point.y

        start_x, start_y = _get_xy(path[0])
        self._exec(
